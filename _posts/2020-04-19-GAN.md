...
![GAN generator](/codes/DGM/GAN_generator.JPG)

**判别器** (discriminator) 是一个函数比如神经网络，用来区分来自数据集的"真实"数据，和来自模型生成器的"伪造"数据。它的目标为最大化 two-sample test。

![GAN discriminator](/codes/DGM/GAN_discriminator.JPG)
...
![mode collapse](/codes/DGM/mode_collapse_target.JPG)
...
![lower semicontinuous](/codes/DGM/lower_semicontinuous.JPG)
...
图片翻译：给定了来自两个领域的样本图片 $\mathcal{X}$ 和 $\mathcal{Y}$。
