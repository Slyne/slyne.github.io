...
举个例子，假设 $Z=1/4X$ 并且 $Z \sim \mathcal{U}[0,2]$, 那么 $p_X(4)$ 为多少? 带入公式 $h(X) = X/4$，所以 $p_X(4) = p_Z(1)h'(4) = 1/2 x 1/4 = 1/8$。

更一般的，假设 $Z$ 是一个均匀随机向量 $[0,1]^n$，$X=AZ$，其中 $A$ 是一个可逆的方阵，它的逆矩阵为 $W=A^{-1}$，那么 $X$ 是怎么分布的？

矩阵 $A$ 是把一个单位超立方体 $[0,1]^n$ 变成了一个超平行体。超立方体和超平行四边形是正方形和平行四边形的更一般式。下图中，我们看到这里的变换矩阵 $\left(\begin{matrix} a & c \\ b & d \\ \end{matrix} \right)$ 把单位正方形变成了一个任意的平行四边形。
...
![flow_generator](/codes/flow_generator.JPG)
...
![flow_transformation](/codes/flow_transformation.JPG)
